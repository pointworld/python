...
'''在正式开始Web开发前，我们需要编写封装一个Web框架，aiohttp已经是一个Web框架了，
为什么我们还需要自己封装一个？原因是从使用者的角度来说，aiohttp相对比较底层'''
...
这些重复的工作可以由框架完成
		...
		编写简单的函数而非引入request和web.Response还有一个额外的好处，就是可以单独测试，否则，需要
		模拟一个request才能测试。
要把一个函数映射为一个URL处理函数 @get和@post
	...
	@post与@get定义类似。
定义RequestHandler类, 封装URL处理函数，调用URL函数，然后把结果转换为web.Response对象
...
最后一步，把很多次add_route()注册的调用：变成自动扫描
...
最后，在app.py中加入middleware、jinja2模板和自注册的支持：
			...
			一个middleware可以改变URL的输入、输出，甚至可以决定不继续处理而直接返回。middleware
			的用处就在于把通用的功能从每个URL处理函数中拿出来，集中放到一个地方。例如，一个记录
			URL日志的logger可以简单定义如下：			
