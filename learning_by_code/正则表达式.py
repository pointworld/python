正则表达式--匹配字符串
概念
	字符串是编程时涉及到的最多的一种数据结构，对字符串进行操作的需求几乎无处不在。比如判断一个字符串是否是合法的Email地址，虽然可以编程提取@前后的子串，
...
如何用字符来描述字符
	...
	匹配可变长的字符--可变长的字符不可单独使用
		'*'--表示任意个字符（包括0个）, 不可单独使用
		'+'表示至少一个字符, 不可单独使用
		'?'--表示0个或1个字符, 不可单独使用
		'{n}'--表示n个字符, 不可单独使用
		'{n,m}'--表示 n至m 个字符, 不可单独使用
...
're模块'--包含所有正则表达式的功能
	...
	'由于Python的字符串本身也用\转义，所以要特别注意'转义问题
	...
	小结
		正则表达式非常强大，要在短短的一节里讲完是不可能的。要讲清楚正则的所有内容，可以写一本厚厚的书了。如果你经常遇到正则表达式的问题，
		你可能需要一本正则表达式的参考书。
	练习
		...
		someone@gmail.com
			...
			>>> email.match('someone@gmail.com')
			<_sre.SRE_Match object; span=(0, 17), match='someone@gmail.com'>
		bill.gates@microsoft.com
			>>> email=re.compile(r'^\w+.\w+@\w+.\w+$')
			>>> email.match('bill.gates@microsoft.com')
			<_sre.SRE_Match object; span=(0, 24), match='bill.gates@microsoft.com'>
		版本二可以验证并提取出带名字的Email地址：	

		<Tom Paris> tom@voyager.org
			>>> email=re.compile(r'^<(\w+\s\w+)>\s(\w+)@\w+.\w+$')
			>>> e=email.match('<Tom Paris> tom@voyager.org')
			>>> e
